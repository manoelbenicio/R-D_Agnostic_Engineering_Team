...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM credential_home_catalog_entry)
       OR EXISTS (SELECT 1 FROM credential_home_catalog_lifecycle) THEN
...
DROP TRIGGER IF EXISTS credential_home_catalog_lifecycle_nonreuse ON credential_home_catalog_lifecycle;
DROP FUNCTION enforce_credential_home_catalog_lifecycle_nonreuse();
DROP TABLE credential_home_catalog_lifecycle;
