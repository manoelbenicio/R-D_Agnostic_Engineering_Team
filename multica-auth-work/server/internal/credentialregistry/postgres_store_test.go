...
func TestPostgresStoreMigrations132To134AtomicAdmission(t *testing.T) {
	...
	resolver := NewFencedResolver(NewPostgresStore(pool))
...
INSERT INTO credential_home_catalog_entry (
    ...
    first_seen_at, last_seen_at, last_full_scan_at, health_watermark, ttl_nanoseconds, retention_deadline
) VALUES (
    '71000000-0000-4000-8003-000000000002',
    '71000000-0000-4000-8003-000000000001', $1, 2, $2, 'codex', false, 'healthy',
    now(), now(), now(), now(), 3600000000000, now() + interval '1 day'
);
...
) VALUES ($15, $13, 1, '{}', repeat('2', 64), 'restart', $1, 'test', 'test-version');
