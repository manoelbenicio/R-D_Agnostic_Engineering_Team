...
func NewPostgresResolver(pool *pgxpool.Pool) *FencedResolver {
	return NewFencedResolver(NewPostgresStore(pool))
}
...
SELECT
    ...
    e.health_watermark IS NOT NULL
        AND e.health_watermark + make_interval(secs => e.ttl_nanoseconds::double precision / 1000000000) >= now()
