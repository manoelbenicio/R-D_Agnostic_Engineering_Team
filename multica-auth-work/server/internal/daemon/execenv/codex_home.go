...
// prepareCredentiallessCodexHome creates task-local directories without
// resolving shared homes, inspecting provider auth, or copying configuration.
...
// WriteCredentiallessCodexConfig installs validated non-secret runtime config.
...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	// `missing field path` and bails out of `thread/start`. The daemon writes the
	...
	// so cross-task and cross-workspace context leaks (issue #3130) cannot
...
//   - account auth present, dst present -> drop dst and re-copy (refresh)
//   - account auth absent,  dst present -> drop dst (account creds removed)
//   - account auth absent,  dst absent  -> no-op (Codex will report logged-out)
//
// Never logs credential contents; logCodexAuthState surfaces file kind only.
