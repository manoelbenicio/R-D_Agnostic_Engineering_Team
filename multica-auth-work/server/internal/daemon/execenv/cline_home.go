...
// prepareCredentiallessClineHome creates only task-local directories for the
// controlled Cline data dir. It does not resolve any account home, inspect
// provider auth, or copy configuration. The controlled OmniRoute providers.json
// carrier is written later by WriteCredentiallessClineConfig after route
// admission. Mirrors prepareCredentiallessCodexHome.
...
// WriteCredentiallessClineConfig installs the non-secret Cline providers.json
// carrier generated by runtimeenv at <clineDataDir>/settings/providers.json.
// The caller must validate the contract before invoking it; the carrier holds
// only the reference sentinel, never a secret value. Mirrors
// WriteCredentiallessCodexConfig.
