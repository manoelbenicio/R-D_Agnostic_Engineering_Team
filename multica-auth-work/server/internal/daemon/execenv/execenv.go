...
type Environment struct {
	...
	CodexHome string
	// KiroDataHome is the path to the per-task isolated XDG_DATA_HOME directory
	// (set only for the kiro provider when a per-account credential is used).
	// The daemon exports it as XDG_DATA_HOME so kiro-cli (Amazon Q fork) reads
	// the account's own data.sqlite3 instead of the shared user store.
	KiroDataHome string
	// AntigravityHome is the path to the per-task isolated HOME directory (set
	// only for the antigravity provider when a per-account credential is used).
...
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	// For Kiro, when a per-account credential dir is provided, prepare an
	// isolated XDG_DATA_HOME seeded (AS-IS) from that account's kiro-cli store.
	...
	if params.Provider == "kiro" && params.CredentialAccountHome != "" {
		...
		if err := prepareKiroHome(kiroDataHome, KiroHomeOptions{AccountHome: params.CredentialAccountHome}, logger); err != nil {
			return nil, fmt.Errorf("execenv: prepare kiro-home: %w", err)
		}
		env.KiroDataHome = kiroDataHome
	}

	// For Antigravity, when a per-account credential dir is provided, prepare an
	// isolated HOME seeded (AS-IS) from that account's antigravity token dir.
	// Empty CredentialAccountHome = shared/global behavior (no isolated home).
...
func Reuse(params ReuseParams, logger *slog.Logger) *Environment {
	...
	// Kiro per-account isolation refreshed on reuse (mirror of Prepare).
	if params.Provider == "kiro" && params.CredentialAccountHome != "" {
		kiroDataHome := filepath.Join(env.RootDir, "kiro-data-home")
		if err := prepareKiroHome(kiroDataHome, KiroHomeOptions{AccountHome: params.CredentialAccountHome}, logger); err != nil {
			logger.Warn("execenv: refresh kiro-home failed", "error", err)
		} else {
			env.KiroDataHome = kiroDataHome
		}
	}

	// Antigravity per-account isolation refreshed on reuse (mirror of Prepare).
	if params.Provider == "antigravity" && params.CredentialAccountHome != "" {
