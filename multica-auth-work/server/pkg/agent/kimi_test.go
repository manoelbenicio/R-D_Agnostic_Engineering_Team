...
// completes: it acks initialize + session/new, records receipt of
// session/prompt, then hangs without ever replying — so the ONLY thing that
// can end the turn is the daemon cancelling runCtx (kimiBackend uses
// exec.CommandContext, which kills the child on cancel). Env files let the test
// observe the child PID and the number of session/prompt dispatches.
func fakeKimiACPCancelScript() string {
	return `#!/bin/sh
echo $$ > "$KIMI_PID_FILE"
while IFS= read -r line; do
  id=$(printf '%s' "$line" | sed -n 's/.*"id":\([0-9]*\).*/\1/p')
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      # Record the dispatch, then hang: never send a PromptResponse. The turn can
      # only end via cancellation-driven process termination.
      printf 'x' >> "$KIMI_PROMPT_FILE"
...
// TestKimiBackendCancellationTerminatesTurnWithoutReplay proves the Kimi
// adapter's half of OpenSpec 8.6: prompt cancellation terminates the active
// turn AND the child process, surfaces a single terminal "aborted" Result, and
// the turn is dispatched exactly once (no replay / re-dispatch of the prompt).
// Disjoint from the gateway Coordinator tests and from the NIM HTTP-adapter
// tests; complements the existing Claude/Codex cancellation coverage.
func TestKimiBackendCancellationTerminatesTurnWithoutReplay(t *testing.T) {
	t.Parallel()

	tempDir := t.TempDir()
	pidFile := filepath.Join(tempDir, "pid.txt")
	promptFile := filepath.Join(tempDir, "prompts.txt")
	fakePath := filepath.Join(tempDir, "kimi")
	writeTestExecutable(t, fakePath, []byte(fakeKimiACPCancelScript()))

	backend, err := New("kimi", Config{
		ExecutablePath: fakePath,
		Logger:         slog.Default(),
		Env:            map[string]string{"KIMI_PID_FILE": pidFile, "KIMI_PROMPT_FILE": promptFile},
	})
	if err != nil {
		t.Fatalf("new kimi backend: %v", err)
	}
	...
	defer cancel()
	// No Model / no ThinkingLevel → the lifecycle goes straight to session/prompt.
	session, err := backend.Execute(ctx, "do work", ExecOptions{Timeout: 30 * time.Second})
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
	go func() {
	...
	// Wait until the fake has received the prompt (the turn is active), then
	// cancel — so cancellation lands during an in-flight turn, not before it.
	deadline := time.Now().Add(5 * time.Second)
	for {
		if b, _ := os.ReadFile(promptFile); len(b) > 0 {
			break
		}
		if time.Now().After(deadline) {
			t.Fatal("fake kimi never received session/prompt")
		}
	...
	case result, ok := <-session.Result:
		...
		if result.Status != "aborted" {
			t.Fatalf("status = %q, want aborted (prompt cancellation must terminate the turn)", result.Status)
		}
	case <-time.After(10 * time.Second):
		t.Fatal("timeout: cancellation did not terminate the turn/process")
	}

	// Exactly one prompt dispatch — the cancelled turn is not re-issued.
	if b, _ := os.ReadFile(promptFile); len(b) != 1 {
		t.Fatalf("session/prompt dispatch count = %d, want 1 (a cancelled turn must not be replayed)", len(b))
	}

	// Process termination (Linux target): the child PID must be gone after cancel.
	if runtime.GOOS == "linux" {
		pid := strings.TrimSpace(string(mustReadKimiFile(t, pidFile)))
		if pid != "" {
			gone := false
			for i := 0; i < 300; i++ {
				if _, statErr := os.Stat("/proc/" + pid); os.IsNotExist(statErr) {
					gone = true
					break
				}
				time.Sleep(10 * time.Millisecond)
			...
			if !gone {
				t.Fatalf("fake kimi process pid=%s still alive; cancellation did not terminate the child", pid)
			}
		}
	}
}

func mustReadKimiFile(t *testing.T, path string) []byte {
	t.Helper()
