...
export async function handleChatCore({
  ...
  if (!stream) {
    ...
    if (isResponsesEndpoint && successConnectionId) {
      const responseId =
        translatedResponse && typeof translatedResponse.id === "string"
          ? translatedResponse.id.trim()
          : "";
      if (!responseId) {
        return createErrorResult(HTTP_STATUS.BAD_GATEWAY, "missing_response_affinity_handle");
      }
      // Synchronous SQLite commit: never return a successful stateful handle
      // whose owner was not persisted.
      try {
        bindResponseConnectionAffinity(responseId, provider, successConnectionId);
      } catch {
        return createErrorResult(HTTP_STATUS.SERVICE_UNAVAILABLE, "continuation_affinity_unavailable");
      }
      const previousResponseId =
        typeof body.previous_response_id === "string" ? body.previous_response_id.trim() : "";
      if (previousResponseId) {
        touchResponseConnectionAffinity(previousResponseId, provider, successConnectionId);
      }
    }
    return {
      success: true,
  ...
  const onStreamComplete = ({
    ...
    if (normalizedStreamStatus === 200) {
      if (isResponsesEndpoint && streamConnectionId) {
        const payload = (clientPayload ?? streamResponseBody) as Record<string, unknown> | null;
        ...
        const responseId =
          typeof payload?.id === "string"
            ? payload.id.trim()
            : typeof nestedResponse?.id === "string"
              ? nestedResponse.id.trim()
              : "";
        if (!responseId) {
          throw new Error("missing_response_affinity_handle");
        }
        try {
          bindResponseConnectionAffinity(responseId, provider, streamConnectionId);
        } catch {
          throw new Error("continuation_affinity_unavailable");
        }
        const previousResponseId =
          typeof body.previous_response_id === "string" ? body.previous_response_id.trim() : "";
        if (previousResponseId) {
          touchResponseConnectionAffinity(previousResponseId, provider, streamConnectionId);
        }
      }
