...
export const DEFAULT_RESPONSE_AFFINITY_TTL_MS = 60 * 60_000;
export const DEFAULT_RESPONSE_AFFINITY_MAX_ENTRIES = 10_000;
...
function responseAffinityKey(responseId: string): string {
  const hash = createHash("sha256")
    .update(`omniroute:responses:v1:${responseId.trim()}`)
    .digest("hex");
  return `${RESPONSE_KEY_PREFIX}${hash}`;
}

export type ResponseConnectionAffinity = {
  provider: string;
  connectionId: string;
};

export function getResponseConnectionAffinity(
  responseId: string,
  now: number = Date.now(),
  ttlMs: number = DEFAULT_RESPONSE_AFFINITY_TTL_MS
): ResponseConnectionAffinity | null {
  if (!responseId?.trim() || !Number.isFinite(ttlMs) || ttlMs <= 0) return null;

  const db = getDbInstance();
  const key = responseAffinityKey(responseId);
  return db.transaction(() => {
    const rows = db
      .prepare(
        `SELECT provider, connection_id AS connectionId, last_seen_at AS lastSeenAt
    ...
    // A response handle must have exactly one owner. Ambiguity fails closed.
    ...
    if (row.lastSeenAt + ttlMs <= now) {
      db.prepare("DELETE FROM session_account_affinity WHERE session_key = ?").run(key);
      return null;
    }
    return { provider: row.provider, connectionId: row.connectionId };
  })();
}

export function bindResponseConnectionAffinity(
  responseId: string,
  provider: string,
  connectionId: string,
  now: number = Date.now(),
  ttlMs: number = DEFAULT_RESPONSE_AFFINITY_TTL_MS,
  maxEntries: number = DEFAULT_RESPONSE_AFFINITY_MAX_ENTRIES
): void {
  if (
    !responseId?.trim() ||
    !provider?.trim() ||
    ...
    throw new Error("invalid_response_affinity_binding");
  }
  ...
  db.transaction(() => {
    db.prepare(
      `DELETE FROM session_account_affinity
        WHERE session_key LIKE ? AND last_seen_at <= ?`
    ).run(`${RESPONSE_KEY_PREFIX}%`, cutoff);

    // Enforce exactly one provider/connection owner per response handle.
    db.prepare("DELETE FROM session_account_affinity WHERE session_key = ?").run(key);
    db.prepare(
      `INSERT INTO session_account_affinity
         (session_key, provider, connection_id, created_at, last_seen_at)
       VALUES (?, ?, ?, ?, ?)`
    ).run(key, provider, connectionId, now, now);

    const { count } = db
      .prepare(
        `SELECT COUNT(*) AS count FROM session_account_affinity
          WHERE session_key LIKE ?`
      )
      .get(`${RESPONSE_KEY_PREFIX}%`) as { count: number };
    const excess = count - maxEntries;
    if (excess > 0) {
      db.prepare(
        `DELETE FROM session_account_affinity
          WHERE rowid IN (
            SELECT rowid FROM session_account_affinity
             WHERE session_key LIKE ?
             ORDER BY last_seen_at ASC, session_key ASC, provider ASC
             LIMIT ?
          )`
      ).run(`${RESPONSE_KEY_PREFIX}%`, excess);
    }
  })();
}

export function touchResponseConnectionAffinity(
  responseId: string,
  provider: string,
  connectionId: string,
  now: number = Date.now()
): boolean {
  if (!responseId?.trim() || !provider || !connectionId) return false;
  const result = getDbInstance()
    .prepare(
      `UPDATE session_account_affinity SET last_seen_at = ?
        WHERE session_key = ? AND provider = ? AND connection_id = ?`
    )
    .run(now, responseAffinityKey(responseId), provider, connectionId);
