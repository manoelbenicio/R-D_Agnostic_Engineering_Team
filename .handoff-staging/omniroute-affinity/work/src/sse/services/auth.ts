...
interface CredentialSelectionOptions {
  ...
  /** Internal continuation owner. Never populate from a northbound header. */
  ...
  try {
    ...
    let forcedConnectionId =
      typeof options.forcedConnectionId === "string" && options.forcedConnectionId.trim().length > 0
        ...
        : null;
    const requiredConnectionId =
      typeof options.requiredConnectionId === "string" &&
      options.requiredConnectionId.trim().length > 0
        ? options.requiredConnectionId.trim()
        : null;
    ...
    if (requiredConnectionId) {
      // Stateful continuations must never be repinned or rotated. The remaining
      // eligibility/quota/cooldown filters still run against this one owner.
      connections = connections.filter((conn) => conn.id === requiredConnectionId);
    } else {
      // #5903: ordinary session affinity may override a reset-aware forced target.
      forcedConnectionId =
        applySessionAffinityPin({
          forcedConnectionId,
          options,
    ...
    if (!requiredConnectionId && forcedConnectionId) {
    ...
    if (connections.length === 0) {
      ...
      if (requiredConnectionId) {
        allConnections = allConnections.filter((conn) => conn.id === requiredConnectionId);
      } else if (forcedConnectionId) {
