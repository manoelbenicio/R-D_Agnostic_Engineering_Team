...
import {
  ...
  type ResponseConnectionAffinity,
...
export async function handleChat(
  ...
  let continuationBinding: ResponseConnectionAffinity | null = null;
  try {
    continuationBinding = previousResponseId
      ? getResponseConnectionAffinity(previousResponseId)
      : null;
  } catch {
    return errorResponse(503, "continuation_affinity_unavailable");
  }
  if (previousResponseId && !continuationBinding) {
    return errorResponse(409, "continuation_unavailable");
  }
  if (sessionId) {
  ...
  if (combo) {
    ...
    const checkModelAvailable = async (
      ...
      if (continuationBinding && provider !== continuationBinding.provider) return false;
      ...
      const creds = await getProviderCredentialsWithQuotaPreflight(
        ...
        resolvedModel,
        {
          ...
          ...(continuationBinding
            ? { requiredConnectionId: continuationBinding.connectionId }
            : {}),
...
async function handleSingleModelChat(
  ...
  runtimeOptions: {
    ...
    continuationBinding?: ResponseConnectionAffinity | null;
  } = {},
  ...
  const continuationBinding = runtimeOptions.continuationBinding ?? null;
  if (runtimeOptions.previousResponseId && !continuationBinding) {
    return errorResponse(409, "continuation_unavailable");
  }
  if (continuationBinding && continuationBinding.provider !== provider) {
    return errorResponse(409, "continuation_unavailable");
  }
  const bypassProviderQuotaPolicy = hasProviderQuotaBypassScope(apiKeyInfo?.scopes);
  const hasForcedConnection =
