...
describe('connectTerminalSocket', () => {
  it('builds ws and wss URLs from the CAO base URL', () => {
    expect(buildTerminalSocketUrl('abcd1234', 'http://127.0.0.1:9889')).toBe(
      'ws://127.0.0.1:9889/terminals/abcd1234/ws'
    );
    expect(buildTerminalSocketUrl('term/with slash', 'https://cao.example.test/base')).toBe(
      'wss://cao.example.test/terminals/term%2Fwith%20slash/ws'
    );
  });
  ...
  it('sets binaryType before returning the socket handle', () => {
    ...
    const handle = connectTerminalSocket('abcd1234', { onBinary: vi.fn() }, 'http://127.0.0.1:9889');

    expect(handle.socket).toBe(FakeWebSocket.instances[0]);
    expect(FakeWebSocket.instances[0]?.url).toBe('ws://127.0.0.1:9889/terminals/abcd1234/ws');
  ...
  it('surfaces 4003 and 4004 close codes as typed reasons', () => {
    ...
    connectTerminalSocket('term-supervisor', { onBinary: vi.fn(), onClose }, 'http://127.0.0.1:9889');
