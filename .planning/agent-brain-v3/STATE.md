...
| G4 | IN_PROGRESS | Accepted: I1/I2 + I3–I5 contracts; GATEWAY CORE RUNTIME; aggregate `SteadyStateFacts`; gateway I3/I4/I5 producers; cleanup no-delete (pB+Gemini); pD L2+AccountHome (pB ACCEPT); Grok A–D (Kiro ACCEPT); MCP guard (ACCEPT; mitigates R23); **ExactEnv physical containment (ACCEPT; residual same-UID TOCTOU needs OS isolation; R22 MITIGATED)**; **gateway projection warm-cache cancellation (ACCEPT)**; **p9 runtime discovery + Windows fail-closed (ACCEPT; R24/R25 closed)**; **RSS Linux lifecycle fix (ACCEPT; EV-G4-08 pin)**; **R26 route preservation (`REVIEW-R26-ACCEPT`)**. pD pane QUOTA-BLOCKED (no reset without owner auth). **51/85** (0.2–0.6 + 8.8 documentary closed; **8.2 REOPENED**; 0.1/0.7 OPEN; 8.1 OPEN; 9.1 STOPPED) | OS-level isolation for residual ExactEnv same-UID TOCTOU; 9.1 thresholds PENDING owner + pB acceptance (STOPPED); all credential/live STOPs | 9.1 readiness (gated; STOPPED until thresholds + pB) → provenance finalize |
| G5 | NOT_STARTED | — | G4 | — |
| G6 | NOT_STARTED | — | G5 + autorização cutover/removal | — |
...
- R26 independently ACCEPTED (`REVIEW-R26-ACCEPT`, 2026-07-18): handler review confirmed exact
  ...
  default test login; no credentials or DB/Docker state were used. Count/gates unchanged: **51/85**;
...
- **PD-08 gate semantics (clarification — does NOT weaken PD-08):** PD-08 has two facets.
  ...
  **subject to every prerequisite and STOP condition in the acceptance checklist**, with thresholds
  ...
  access is absolute; count is **51/85** (0.1/0.7 OPEN, 8.1 OPEN, 9.1 STOPPED).
...
- Os portões da seção 7.4, assinaturas 7.5, remoção do Prodex, cutover default e tiers 50/100
...
- G4 remains IN_PROGRESS. The earlier `SteadyStateFacts`, aggregate, gateway-producer, runtime-catalog,
  ...
  **GATED (not now):** 9.1 readiness (owner decisions A–F); SC01–SC10 / reset-redeem (owner/G5).
  Preserve **51/85** unless a further evidence-backed closure exists (0.2–0.6 + 8.8 closed; 8.2/1.5/1.6 reopened;
