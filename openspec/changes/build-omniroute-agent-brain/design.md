...
The final local source preserves both accepted transport bindings and incorporates stable physical
allocator semantics through credential-account-home REQ-24..35. The later ROOT OmniRoute-only
proposal/spec reductions are classified as historical divergence, not a superseding authority. The
unique later `proc_id` evidence is retained: because `agent.Result` exposes no real process ID, CLI
span `proc_id` remains absent rather than fabricated; all other recorder links remain active.

Source composition and local gates are complete. Commit, push, deployment and restart are outside
this candidate and remain separately governed.
