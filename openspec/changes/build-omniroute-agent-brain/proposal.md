...
- **Final ownership clarification (owner, 2026-07-21):** OmniRoute alone owns all inference authentication, credentials, token/window limits (including five-hour limits), account lifecycle/selection, refresh/revocation, quota, 401/403, 429/5xx, retry and provider/account fallback. Multica/Agent Brain neither implements nor tests those internals; it consumes only route/model intent plus opaque OmniRoute readiness/result. Tasks `8.5–8.7` are external OmniRoute certification, not Main Brain P0 lanes. Prodex remains future Phase 3/HOLD and is not a current fallback.

- **Production-integrity and evidence-reuse directive (owner, 2026-07-21):** production-reachable mocks, placeholder records, fake-success fallbacks, QA-only routes and synthetic runtime defaults are prohibited. Test-only fixtures and safety guardrails remain isolated and may be preserved. Acceptance reuses valid evidence from the same deployed version/configuration; a behavior is executed again only when implementation/integration changed, evidence is missing or stale, or a distinct material-risk scenario remains uncovered. Live validation performed during integration is the acceptance run—there is no duplicate QA-A/QA-B, broad regression, or second live-acceptance phase without a documented reason.

- **P0 functional execution scope (owner directive D-V3-29, superseded scope clarification):** the only pending tasks consuming Main Brain P0 lanes are `5.6–5.8` and `8.1–8.2`; `8.5–8.7` are OmniRoute-owned external certification. P0 exit is a working squad/project/Kanban-task flow through Agent Brain and approved OmniRoute routes to a terminal result. Chat and end-to-end observability are P2 for implementation priority; observability remains a later blocker for capacity certification/cutover, not initial functional use.
