...
#   ./start.sh dev                     # vite dev server with HMR (real GO Core required)
...
#   4. Probe the GO Core server at $VITE_GO_CORE_BASE_URL (default http://127.0.0.1:8080).
...
#        show GO Core offline; canvas deploys / terminals / flows will fail until GO Core
...
#
# GO Core is an external dependency by design (master spec §13; cloud-runtime-deployment
# is post-launch). This script CANNOT install GO Core. If you have a GO Core Docker image
# or local install path, set $GO_CORE_DOCKER_IMAGE or $GO_CORE_START_CMD and the script will
...
: "${DEV_PORT:=5173}"
: "${LOG_DIR:=/tmp}"
: "${GO_CORE_DOCKER_IMAGE:=agentverse-runtime:latest}"   # auto-built from infra/runtime/Dockerfile if missing
: "${GO_CORE_DOCKERFILE:=infra/runtime/Dockerfile}"
: "${GO_CORE_START_CMD:=}"      # e.g. "uv run go-core serve --port 8080"
...
GO_CORE_LOG="$LOG_DIR/agentverse-go-core.log"
...
Environment overrides:
  VITE_GO_CORE_BASE_URL    Runtime endpoint to probe       (default http://127.0.0.1:8080)
  PREVIEW_PORT         Production preview port         (default 4173)
  DEV_PORT             Vite dev server port            (default 5173)
  GO_CORE_DOCKER_IMAGE     Image to start runtime          (default agentverse-runtime:latest)
  GO_CORE_DOCKERFILE       Source Dockerfile if rebuilding (default infra/runtime/Dockerfile)
  GO_CORE_START_CMD        Shell command to start runtime  (alternative to docker)
...
Examples:
  ./start.sh
  ./start.sh dev
  GO_CORE_DOCKER_IMAGE=agentverse-runtime:dev ./start.sh
...
start_go_core_if_configured() {
  step "Probing GO Core at $VITE_GO_CORE_BASE_URL"
  local code; code=$(probe "$VITE_GO_CORE_BASE_URL/health" 3)
  if [[ "$code" == "200" ]]; then
    ok "GO Core is reachable (HTTP $code)"
    return 0
  fi

  warn "GO Core is NOT reachable (HTTP $code)"

  if [[ -n "$GO_CORE_DOCKER_IMAGE" ]]; then
    step "Starting GO Core from Docker image: $GO_CORE_DOCKER_IMAGE"
    if ! command -v docker >/dev/null 2>&1; then
      err "docker not found but GO_CORE_DOCKER_IMAGE is set"
      return 1
    fi
    # Auto-build the image from infra/go-core/Dockerfile if it isn't already present
    if ! docker image inspect "$GO_CORE_DOCKER_IMAGE" >/dev/null 2>&1; then
      if [[ -f "$GO_CORE_DOCKERFILE" ]]; then
        step "Image $GO_CORE_DOCKER_IMAGE not found locally — building from $GO_CORE_DOCKERFILE"
        docker build -t "$GO_CORE_DOCKER_IMAGE" -f "$GO_CORE_DOCKERFILE" . >>"$GO_CORE_LOG" 2>&1 || {
          err "docker build failed (see $GO_CORE_LOG)"; return 1; }
        ok "image built"
      else
        err "image $GO_CORE_DOCKER_IMAGE not present and no Dockerfile at $GO_CORE_DOCKERFILE"
        return 1
      fi
    fi
    docker rm -f agentverse-runtime agentverse-go-core >/dev/null 2>&1 || true
    docker run -d --name agentverse-runtime \
      ...
      -v agentverse-runtime-state:/root/.go-core \
      "$GO_CORE_DOCKER_IMAGE" >>"$GO_CORE_LOG" 2>&1 || { err "docker run failed (see $GO_CORE_LOG)"; return 1; }
    sleep 3
    local i
    for i in $(seq 1 30); do
      code=$(probe "$VITE_GO_CORE_BASE_URL/health" 2)
      if [[ "$code" == "200" ]]; then ok "GO Core is up (HTTP 200)"; return 0; fi
      sleep 1
    done
    err "GO Core did not become healthy within 30s — check $GO_CORE_LOG"
    return 1
  fi

  if [[ -n "$GO_CORE_START_CMD" ]]; then
    step "Starting GO Core via GO_CORE_START_CMD"
    spawn_detached "$GO_CORE_LOG" bash -lc "$GO_CORE_START_CMD"
    local i
    for i in $(seq 1 30); do
      code=$(probe "$VITE_GO_CORE_BASE_URL/health" 2)
      if [[ "$code" == "200" ]]; then ok "GO Core is up (HTTP 200)"; return 0; fi
  ...
  cat <<EOF
  ${C_YEL}!${C_OFF} GO Core is required for canvas deploys, terminal streaming, and flows.
    Per master spec §13 it is an EXTERNAL service. To start it, do ONE of:

      1) Set GO_CORE_DOCKER_IMAGE if you have a GO Core image:
           GO_CORE_DOCKER_IMAGE=go-core-server:latest ./start.sh

      2) Set GO_CORE_START_CMD to a local command (e.g. uv/pip-installed GO Core):
           GO_CORE_START_CMD="uv run go-core serve --port 8080" ./start.sh

      3) Start GO Core yourself in another terminal at $VITE_GO_CORE_BASE_URL.

    The SPA will start anyway and the Health page will show GO Core offline.
    See docs/go-core-cors.md for the env vars GO Core needs to allow this SPA.
...
cmd_status() {
  ...
  printf "  GO Core     (%s) : HTTP %s\n" "${VITE_GO_CORE_BASE_URL##*/}" "$(probe "$VITE_GO_CORE_BASE_URL/health" 2)"
  hr
  step "Processes"
  pgrep -af "vite preview" 2>/dev/null | sed 's/^/  preview  /' || true
  pgrep -af "vite dev|vite\$"   2>/dev/null | sed 's/^/  dev      /' || true
  if command -v docker >/dev/null 2>&1; then
    docker ps --filter name=agentverse-go-core --format '  go-core      {{.ID}} {{.Image}} {{.Status}}' 2>/dev/null || true
  fi
  hr
}
...
cmd_stop() {
  ...
  if command -v docker >/dev/null 2>&1; then
    docker rm -f agentverse-go-core >/dev/null 2>&1 || true
  fi
  ok "stopped"
}
