...
2. **Check-in em disco — ANTES e DEPOIS de CADA task (obrigatório, sem exceção).** Para TODA task (não é uma vez por sessão): escreva `.deploy-control/CHECKIN_<agentname>_<UTC-ISO8601>_START.md` **ANTES** de iniciar a task (escopo, arquivos, deps, riscos) e `.deploy-control/CHECKIN_<agentname>_<UTC-ISO8601>_DONE.md` **DEPOIS** de concluí-la (o que fez, arquivos alterados, evidência de build/test). Múltiplas tasks = múltiplos pares START/DONE. **Nunca** comece nem finalize uma task sem o check-in correspondente escrito em disco.
