...
**Current ORQ2 credential-home blocker (read-only audit at 2026-08-01 07:05 BRT):** proliferation recurred while the validated allocator correction remained undeployed. Eight directories now exist: `slot-184`, `slot-185`, `slot-186`, `slot-187`, `slot-188`, `slot-192`, `slot-193`, and `slot-194`. A metadata-only Linux `/proc` audit found live process references for every directory, respectively 6, 6, 6, 6, 6, 4, 4, and 6. No directory is safe to delete, and no cleanup was performed. Source allocator and harness validation does not remediate the installed behavior; deployment and restart remain unauthorized. This recurrence is an additional current restart-readiness HOLD blocker and supersedes the earlier single-slot claim for current-state decisions.

The installed ORQ2 allocator still contains the incident-causing behavior: it keys slots by ephemeral Herdr terminal identity and creates/persists a fresh fallback UUID whenever lookup fails. Cleanup corrected the immediate disk condition, and the synchronized source now has a reviewed correction requiring a nonzero agent UUID plus a 64-character lowercase hexadecimal provider-subscription fingerprint, failing before allocation when identity is missing or invalid, removing Herdr/pane/TTY/PID/random fallback identity, reconciling one physical folder per stable binding, and protecting live OS references. Syntax, harness, fail-closed non-root reconciliation, and independent source review reported pass. The installed allocator and live `slot-185` were deliberately not modified, so operational exposure remains until an authorized deployment.

**Current restart conclusion:** disk pressure, source allocator correction, shared C2/C3/SPE-10 source integration, strict OpenSpec, all local non-database and disposable-PostgreSQL gates, accepted-tree companion lanes I/J/K/L, canonical self-contained migration-135 DDL, and deterministic direct sqlc generation are complete in their bounded scopes, but full Multica restart readiness is **not certified**. Authorized target migration-135 restoration, 59-file byte verification, bounded generated-output verification, preservation scans, consolidated target Go/C4/SPE-7 gates, and an approved integration commit remain open. No push, deployment, production mutation, or restart is authorized or has occurred.
...
This comparison indicates that the observed slot proliferation is currently localized to ORQ2's installed credential-isolation workflow and history, not a confirmed fleet-wide condition. It does not eliminate the need to inspect synchronized source and deployment coverage before rollout.
...
- ORQ2: one live agent pane, `w5:p9`, label `Kiro-Opus5-SENIOR-GENERAL-TECH-MANAGER`, with live `kiro-cli`/`kiro-cli-chat` processes.
- ORQ2: all other named panes had only idle `bash` foreground processes and were not counted as online agents.
- ORQ1: no matching live agent processes; Herdr pane listing was unavailable.
...
- The source allocator syntax check, full harness, fail-closed non-root reconcile test, and independent two-file review passed without changing the installed allocator or `slot-185`.
...
### 10.3 Still open or blocked

- Authorized target restoration remains: copy only the newly canonical manifest version of `server/migrations/135_migration_checksums.up.sql` into `/tmp/multica-n1-c1/multica-auth-work`, verify all 59 ordinary hashes against authoritative manifest `52b5c6aaa8320613134638335ce8152951f9d1b715a4cfc364fa4f65bce9b53a`, run sqlc v1.31.1, accept only generated hashes `a0b5ac3022aa722b2ac3db85a6086634e57329e811f60c4e4a4a74507862b896` and composed `79e81bb2abcd8d98a0cd01ef5a42c32b63b94df2d283a35adcae284bef449acb` with zero further spill, preserve migration 129, `task_usage.sql`, additive router/main/auth routes, and all accepted repairs, then run protected-path/placeholder scans and consolidated target Go/C4/SPE-7 gates.
- N0v2 is absent from the ORQ2 local object graph. Final reviewed changes must be imported selectively on the coordination host; the detached ORQ2 root cannot certify the release.
- The installed ORQ2 allocator remains unchanged. Source acceptance does not authorize deployment or restart.
- No approved integration commit, push, deployment, or restart has occurred.

## 11. Risk assessment

### Critical/high risks

1. **Silent functional deletion if rejected N0 is reused.** Mitigation: forensic-only branch, never push; use exact N0v2 SHA on the coordination host and selective import only.
2. **Credential-slot regrowth on ORQ2 before deployment.** Cleanup and source correction are complete, but the installed allocator remains defective. A restart or identity churn can still allocate new slots until authorized rollout.
3. **Incomplete target restoration.** Shared source composition and all local database/sqlc/build evidence pass, but the newly canonical migration-135 file has not yet been restored and verified in the accepted target tree.
4. **Accepted-repair preservation during restoration.** I/J/K/L and additive router/main/auth-route blockers are closed. The residual risk is overwrite during the bounded target restoration or generation; all 59 manifest hashes, the two authorized generated hashes, preservation scans, and final consolidated tests must detect it.
5. **Premature restart.** Passing local source, database, direct-sqlc, compile, and build gates do not replace target migration restoration, manifest/generated hash verification, preservation scans, final consolidated target gates, and deployment authorization.

### Medium risks

1. **Registry/filesystem divergence.** ORQ2 has one folder but historical registry entries remain. Reconciliation must be designed so stale metadata cannot resurrect slots.
...
| CAPA-03 | Selectively integrate C1 without resolver overwrite | In progress on coordination host | Accepted tree preserved; no whole-branch merge |
| CAPA-04 | Import C2 schema/query work and regenerate sqlc | Implemented; final integration pending | Migration 128 exact; pinned generation/build passed |
...
| CAPA-06 | Integrate C3/C4 and remaining synchronized work | In progress | C3 owned gates, C4 final source tests/typechecks with nine-file byte identity, and SPE-7 28/28 Git-free passed; shared routing is composed and must be preserved through bounded target restoration and final gates |
| CAPA-07 | Remove unreferenced ORQ2 folders | Complete | Only active `slot-185` remains before/after `/proc` verification |
...
| CAPA-10 | Reconcile stale registry entries safely | Complete in source; operational rollout pending | Registry v2, live-reference protection, in-place `slot-185` adoption |
| CAPA-11 | Add cardinality/disk monitoring | Pending | Alert when folders differ from active bindings or grow unexpectedly |
| CAPA-12 | Close systemic companions and consolidated gates | Local companions/DB/composition complete; target restoration pending | I/J/K/L, additive composition, authoritative manifest, corrected runner, local DB, direct sqlc, compile, and build gates passed; bounded target restoration and consolidated gates remain |
...
**Decision: NOT READY for full restart or deployment.**

The emergency storage condition is corrected and the accepted additive base is safe, but the release tree is not yet complete. Restart certification requires all of the following:

1. Preserve all accepted I/J/K/L files, additive router/main/auth routes, declarations, and passing package gates during the bounded migration-135 target restoration.
2. Restore only authoritative `server/migrations/135_migration_checksums.up.sql` into `/tmp/multica-n1-c1/multica-auth-work`; do not blind-overwrite other accepted files.
3. Preserve the accepted clean disposable-PostgreSQL migration, reservation, non-race, vet, and race evidence; do not create another runtime unless separately authorized.
4. Verify all 59 ordinary hashes against manifest `52b5c6aaa8320613134638335ce8152951f9d1b715a4cfc364fa4f65bce9b53a`, while preserving accepted migration 129 and `task_usage.sql` byte-for-byte.
5. Run pinned sqlc v1.31.1 twice and accept only deterministic generated `runtime_manager.sql.go` `a0b5ac3022aa722b2ac3db85a6086634e57329e811f60c4e4a4a74507862b896` and composed `models.go` `79e81bb2abcd8d98a0cd01ef5a42c32b63b94df2d283a35adcae284bef449acb`; retain both `TaskUsage` price fields/types/JSON tags and `SchemaMigration`, with all other declarations unchanged.
6. Reverify migration 128 blobs and preserve accepted strict OpenSpec, SPE-7 Git-free 28/28, C4 final source, and ORQ2 shared-source evidence.
7. Run consolidated build, package, race where required, placeholder/forbidden-pattern, protected-path, and documentation/link gates on the final accepted tree.
8. Record an approved local integration commit SHA only after all gates pass.
9. Obtain explicit owner authorization before any push, deployment, allocator installation, or restart.
...
- Minimize ORQ2 terminal/session churn until stable identity is deployed because the source defect can recreate folders.
- If disk cleanup is required again, rederive the active set from `/proc`, hold the registry lock, delete exact direct paths only, and verify before/after.
...
The investigation prevented an unsafe deletion-heavy integration base from advancing, established a reproducible additive base on the coordination host, preserved exact migration 128, restored ORQ2 disk headroom without disrupting the only live credential home, and produced a reviewed source correction for stable credential-home identity and zero historical physical folders. Subsequent bounded reconciliation cleared the original credential-registry, ExactEnv, delivery-observer, agent, realtime, handler, and full-build blockers that have reported passing evidence. Release nevertheless remains **HOLD / NOT READY**: all local source, corrected runner/manifest, migration, reservation, non-race, vet, race, direct sqlc, compile, and build gates passed, and the shared-source composition blocker is resolved, but target restoration under authoritative manifest `52b5c6aaa8320613134638335ce8152951f9d1b715a4cfc364fa4f65bce9b53a`, 59-file byte verification, target sqlc regeneration with only the two authorized generated hashes, preservation checks for additive router/main/auth routes and accepted repairs, protected-path/placeholder checks, and consolidated target Go/C4/SPE-7 gates are not yet evidenced; and no integration commit has been created. The detached ORQ2 root is not the release tree. Until every listed gate passes and the owner explicitly authorizes the action, the correct posture is unpushed, undeployed, and not restart-certified.
