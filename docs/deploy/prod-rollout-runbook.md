...
## 10. Main Brain & OmniRoute Provisioning Governance (Wave 3)

### Governance & Prerequisites

- OmniRoute is the sole router owner for provider models; Main Brain operates in credentialless mode with no provider-native credentials or auth homes in child environments.
- All deployment actions must be recorded with scrubbed metadata in `.deploy-control/evidence/`.
- No deployment action may modify `.planning/` or product code without prior approval.

### OmniRoute / Main Brain Provisioning Note

Production values and secret references are provisioned out of band. Deployment of OmniRoute must adhere to single-router invariants and credentialless child boundaries.
