...
In parallel, ORQ2 accumulated 30 physical credential-slot folders because the installed allocator used ephemeral Herdr/process identity and random fallback UUIDs. A `/proc` audit identified `slot-185` as the only live slot, and the other 29 folders were removed under lock with filesystem-boundary protection. The installed allocator was not hot-edited.
