...
- SSH: `ssh manoelneto-laptop` (alias) ou `ssh dataops-lab@192.168.1.27`. Host na **LAN (mesma rede local)**, IP **`192.168.1.27`**, User `dataops-lab`, Port 22.
- **IP LAN/PROD do host:** `192.168.1.27` (do `ping -a manoelneto-laptop`). Para conectar via LAN, trocar o `HostName` no `~/.ssh/config` para `192.168.1.27`.
