...
Read [references/cli-reference.md](references/cli-reference.md) when the requested operation is outside the core workflows below. Read [references/socket-api.md](references/socket-api.md) only for raw protocol clients, long-lived event subscriptions, declarative agent views, pane graphics, or plugin protocol work. Refresh uncertain syntax and method availability against the installed binary and its schema.
...
- `detection`: the plain-text bottom-buffer snapshot used for agent detection. Herdr 0.7.5 exposes it through both `pane read` and `agent read`.
